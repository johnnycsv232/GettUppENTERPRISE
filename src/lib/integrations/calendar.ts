...
export async function getAvailableSlots(
  ...
  try {
    ...
    const response = await fetch(
      `${CAL_CONFIG.API_BASE}/slots?${params}`,
      {
        headers: {
 ...
 */
export async function createBooking(booking: BookingRequest): Promise<CalendarEvent | null> {
  const calApiKey = process.env.CAL_API_KEY;
...
export async function cancelBooking(
  ...
  try {
    const response = await fetch(`${CAL_CONFIG.API_BASE}/bookings/${bookingUid}/cancel`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${calApiKey}`,
        'cal-api-version': '2024-08-13',
...
export async function rescheduleBooking(
  ...
  try {
    const response = await fetch(`${CAL_CONFIG.API_BASE}/bookings/${bookingUid}/reschedule`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${calApiKey}`,
        'cal-api-version': '2024-08-13',
 ...
 */
export async function getBooking(bookingUid: string): Promise<CalendarEvent | null> {
  const calApiKey = process.env.CAL_API_KEY;
  ...
  try {
    const response = await fetch(`${CAL_CONFIG.API_BASE}/bookings/${bookingUid}`, {
      headers: {
        'Authorization': `Bearer ${calApiKey}`,
        'cal-api-version': '2024-08-13',
      },
