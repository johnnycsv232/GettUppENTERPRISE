...
export async function logPaymentEvent(
  action: 'payment.created' | 'payment.completed' | 'payment.failed' | 'payment.refunded',
  paymentId: string,
  amount: number,
  customerId?: string,
  metadata?: Record<string, unknown>
 ...
 * Get logs by user
 */
export function getLogsByUser(userId: string, limit: number = 50): AuditLogEntry[] {
  return auditLogs
 ...
 */
export function getLogsByAction(action: AuditAction, limit: number = 50): AuditLogEntry[] {
  return auditLogs
