...
const envSchema = z.object({
	...
	NEXT_PUBLIC_FIREBASE_API_KEY: z
		.string()
		.min(1, "❌ NEXT_PUBLIC_FIREBASE_API_KEY not set. Add to .env.local"),
	NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN: z
		.string()
		.min(1, "❌ NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN not set"),
	NEXT_PUBLIC_FIREBASE_PROJECT_ID: z
		.string()
		.min(1, "❌ NEXT_PUBLIC_FIREBASE_PROJECT_ID not set"),
	NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET: z
		.string()
		.min(1, "❌ NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET not set"),
	NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID: z
		.string()
		.min(1, "❌ NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID not set"),
	NEXT_PUBLIC_FIREBASE_APP_ID: z
		.string()
		.min(1, "❌ NEXT_PUBLIC_FIREBASE_APP_ID not set"),

	// Firebase Admin SDK (server-only)
	FIREBASE_ADMIN_PROJECT_ID: z
		.string()
		.min(1, "❌ FIREBASE_ADMIN_PROJECT_ID not set. Add to Vercel secrets."),
	FIREBASE_ADMIN_CLIENT_EMAIL: z
		.string()
		.email("❌ FIREBASE_ADMIN_CLIENT_EMAIL must be valid email"),
	...
	UPSTASH_REDIS_REST_TOKEN: z
		.string()
		.min(1, "❌ UPSTASH_REDIS_REST_TOKEN not set")
		.optional(),
