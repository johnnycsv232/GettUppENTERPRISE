...
import { Body, GlassCard, H1, H2, MagneticButton } from "@/components/ui";
...
export default function SchedulePage() {
	...
	const [selectedTier, setSelectedTier] = useState<
		keyof typeof PRICING_TIERS | null
	>(null);
					...
					{step === "plan" && (
						...
						>
							{Object.entries(PRICING_TIERS)
								.filter(([key]) => key !== "pilot")
								.map(([key, tier]) => (
									<PlanCard
										key={key}
										tier={tier}
										onSelect={() =>
											handleTierSelect(key as keyof typeof PRICING_TIERS)
										}
									/>
								))}
						</motion.div>
						...
						>
							...
							<GlassCard className="min-h-[600px] flex items-center justify-center">
								...
								<div className="text-center">
									...
									<Body className="mb-8">
										We'll sync this with your Johnny Cage's calendar.
									</Body>
									{/* Simulated Date Selection for MVP */}
									<div className="grid grid-cols-2 md:grid-cols-3 gap-4">
										{["Fri Oct 27", "Sat Oct 28", "Fri Nov 3", "Sat Nov 4"].map(
											(date) => (
												<button
													key={date}
								...
								<div className="space-y-4 mb-8">
									...
									<div className="flex justify-between items-center p-4 bg-white/5 rounded-lg border border-brand-gold/30">
										...
										<span className="text-2xl font-bold text-brand-gold">
											{
												PRICING_TIERS[
													selectedTier as keyof typeof PRICING_TIERS
												]?.priceDisplay
											}
										</span>
									</div>
...
function StepIndicator({
	...
	return (
		<div
			className={`flex flex-col items-center gap-2 ${active ? "opacity-100" : "opacity-50"}`}
		>
			<div
				className={`
	...
	return (
		<GlassCard className="relative flex flex-col h-full hover:border-brand-gold/50 transition-colors group">
			...
			<MagneticButton
				onClick={onSelect}
				variant="outline"
				className="w-full group-hover:bg-brand-gold group-hover:text-brand-ink transition-colors"
			>
				Select Plan
			</MagneticButton>
		</GlassCard>
