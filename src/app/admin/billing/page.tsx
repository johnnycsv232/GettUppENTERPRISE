...
export default function BillingPage() {
	const [selectedTab, setSelectedTab] = useState<"subscriptions" | "invoices">(
		"subscriptions",
	);
			...
			<div className="max-w-7xl mx-auto">
				...
				<div className="flex items-center justify-between mb-8">
					...
					<div className="flex gap-3">
						...
						>
							<RefreshCw
								className={`w-4 h-4 mr-2 inline ${syncing ? "animate-spin" : ""}`}
							/>
							Sync Stripe
						</MagneticButton>
								...
								<tbody className="text-sm">
									{SUBSCRIPTIONS.map((sub) => (
										<tr key={sub.id} className="border-b border-white/5">
											...
											<td className="py-4">
												<span
													className={`px-2 py-1 rounded text-xs border ${STATUS_STYLES[sub.status]}`}
												>
													{sub.status === "active" && (
								...
								<tbody className="text-sm">
									{RECENT_INVOICES.map((inv) => (
										<tr key={inv.id} className="border-b border-white/5">
											...
											<td className="py-4">
												<span
													className={`px-2 py-1 rounded text-xs ${STATUS_STYLES[inv.status]}`}
												>
													{inv.status === "paid" && (
														<CheckCircle className="w-3 h-3 inline mr-1" />
													)}
											...
											<td className="py-4">
												<a
													href={inv.invoiceUrl}
													className="text-[var(--brand-gold)] hover:underline text-sm"
												>
