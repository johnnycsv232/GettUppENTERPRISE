...
export default function AdminPage() {
	...
	if (loading) {
		return (
			<main className="min-h-screen bg-[var(--brand-ink)] text-white flex items-center justify-center">
				<div className="text-[var(--brand-gold)] text-xl animate-pulse">
					Loading dashboard...
				</div>
			</main>
		);
	...
	const cards = [
		...
		},
		{
			...
			description: "Currently subscribed venues",
		},
		{
			label: "Pilots This Month",
			value: metrics.pilotsThisMonth,
			description: "New pilot trials started",
		},
		{
			label: "Conversion Rate",
			value: `${metrics.conversionRate}%`,
			description: "Pilot → Retainer conversion",
		},
	];
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-8">
					{cards.map((card) => (
						<GlassCard key={card.label} padding="lg">
							...
							<div className="text-4xl font-bold text-[var(--brand-gold)] mt-2">
								{card.value}
							</div>
							<Body className="text-gray-500 text-xs mt-2">
								{card.description}
							</Body>
						</GlassCard>
					))}
