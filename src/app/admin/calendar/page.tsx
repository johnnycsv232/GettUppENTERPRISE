...
export default function CalendarPage() {
	...
	const [_showNewShootModal, setShowNewShootModal] = useState(false);
				...
				<div className="grid lg:grid-cols-3 gap-6">
					...
					<div className="lg:col-span-2">
                      ...
                      `}
										>
											<span
												className={`text-sm ${isToday ? "text-[var(--brand-gold)] font-bold" : ""}`}
											>
												{day}
											</span>
											{daysShoots.length > 0 && (
							...
							.map((shoot) => (
								<GlassCard key={shoot.id} padding="sm">
									<div className="flex items-start justify-between">
										...
										<span
											className={`px-2 py-1 rounded text-xs font-medium border ${TIER_COLORS[shoot.tier]}`}
										>
					...
					<div className="grid md:grid-cols-2 gap-4 text-sm font-mono">
						<div>
							...
							<code className="text-[var(--brand-gold)]">
								firebase firestore:export shoots
							</code>
						</div>
						<div>
