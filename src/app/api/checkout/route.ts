...
export async function POST(request: NextRequest) {
  ...
  const rateLimit = checkRateLimit(`checkout:${clientIp}`, { maxRequests: 10, windowMs: 60 * 1000 });
  ...
  try {
    ...
    const priceId = STRIPE_PRODUCTS[tier.toUpperCase() as keyof typeof STRIPE_PRODUCTS];
    ...
    if (!priceId || !priceId.startsWith('price_')) {
       ...
       lineItem.price_data = {
          ...
          product_data: { 
            ...
            description: tier === 'pilot' ? 'One-time Pilot Night' : 'Monthly Content Retainer'
          },
