...
const leadSchema = z.object({
  ...
  source: z.enum(['instagram_dm', 'website_form', 'referral', 'qr']).default('website_form'),
...
export async function POST(request: NextRequest) {
  ...
  const rateLimit = checkRateLimit(`leads:${clientIp}`, { maxRequests: 5, windowMs: 60 * 1000 }); // 5 per minute
  ...
  return withAuth(request, async () => {
    try {
      ...
      const leads = snapshot.docs.map(doc => ({ 
        ...
        lastContactAt: doc.data().lastContactAt?.toDate?.()?.toISOString() || null,
        nextActionAt: doc.data().nextActionAt?.toDate?.()?.toISOString() || null,
      }));
