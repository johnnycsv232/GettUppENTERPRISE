...
import { Body, GlassCard, H1, MagneticButton, Small } from "@/components/ui";
...
function LoginForm() {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			// Get the real Firebase ID token (JWT) for middleware validation
			const token = await userCredential.user.getIdToken();

			// Set auth cookie with real JWT for middleware
			// HttpOnly cannot be set from client-side, but this enables edge middleware validation
				...
				<GlassCard>
					...
					<div className="relative my-8">
						...
						<div className="relative flex justify-center">
							<span className="px-4 bg-[var(--brand-ink)] text-sm text-gray-500">
								or
							</span>
						</div>
					</div>
