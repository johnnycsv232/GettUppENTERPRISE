...
export function H3({ children, className = '' }: TypographyProps) {
  return (
    <h3 className={`font-heading text-xl font-semibold text-brand-gold ${className}`}>
      {children}
    </h3>
  );
}
...
export function Label({ children, className = '' }: TypographyProps) {
  return (
    <span className={`text-xs font-medium uppercase tracking-wider text-gray-400 ${className}`}>
      {children}
    </span>
  );
}
