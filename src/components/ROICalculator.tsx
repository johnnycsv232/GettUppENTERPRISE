...
import { Calculator, TrendingUp, Image, DollarSign } from 'lucide-react';
...
export function ROICalculator() {
  ...
  const costPerPost = tierData.photos > 0 
    ? tierData.price / tierData.photos 
    : 0;
  
  const costPerShoot = tierData.shoots > 0
    ? tierData.price / tierData.shoots
    : 0;
  
  const projectedLift = !isNaN(currentMetric) && currentMetric > 0
  ...
  const customersNeeded = tierData.price > 0 
    ? Math.ceil(tierData.price / avgCustomerValue)
    : 0;
      ...
      <div className="space-y-5">
        ...
        <div>
          ...
          >
            ...
            <option value="t3">Tier 3: VIP Partner - $995/mo</option>
          </select>
        </div>
        ...
        <div className="pt-5 border-t border-white/10 space-y-4">
          ...
          <div className="bg-brand-gold/10 border border-brand-gold/30 rounded-lg p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm text-brand-gold font-semibold">Projected 32% Lift</span>
            </div>
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-bold text-white">{projectedTotal.toLocaleString()}</span>
          ...
          <div className="text-center text-sm text-gray-400">
            <span className="text-brand-gold font-semibold">{customersNeeded} new regulars</span>
            {' '}pays for this tier
          </div>
        </div>
      </div>
