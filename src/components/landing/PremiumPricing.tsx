...
export function PremiumPricing() {
	...
	return (
		<section
			ref={ref}
			className="py-32 px-6 bg-brand-ink relative overflow-hidden"
		>
			{/* Background */}
			<div className="absolute inset-0">
				<div className="absolute top-0 left-1/4 w-96 h-96 bg-brand-gold/5 rounded-full blur-[150px]" />
				<div className="absolute bottom-0 right-1/4 w-96 h-96 bg-brand-ai/5 rounded-full blur-[150px]" />
			</div>
			...
			<div className="max-w-7xl mx-auto relative">
				...
				>
					...
					<p className="text-xl text-gray-400 max-w-2xl mx-auto">
						Pilot night proves it works. These are the subscription engines that
						keep your weekends heavy.
					</p>
				</motion.div>
					...
					{tiers.map((tier, i) => (
						...
						>
							...
							>
								{/* Glow effect */}
								<div
									className={`absolute -inset-1 bg-gradient-to-r ${tier.gradient} rounded-3xl opacity-0 group-hover:opacity-20 blur-xl transition-opacity -z-10`}
								/>

								{/* Icon */}
								<div
									className={`w-12 h-12 rounded-xl bg-gradient-to-r ${tier.gradient} flex items-center justify-center text-white mb-4`}
								>
									{tier.icon}
								</div>
								...
								{/* Tier info */}
								<div className="mb-6">
									<p className="text-sm text-[#D9AE43] font-medium mb-1">
										{tier.tagline}
									</p>
				...
				>
					All plans include non-exclusive licensing. Cancel anytime. MN sales
					tax exempt for advertising services.
				</motion.p>
			</div>
		</section>
	);
