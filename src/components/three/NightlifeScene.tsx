...
function FloatingShard({
	...
	useFrame((state) => {
		...
		meshRef.current.position.y =
			position[1] + Math.sin(t * speed + rotationOffset) * 0.5;
	});
...
function SceneContent() {
	return (
		<>
			...
			{/* GLTF Models from user's collection */}
			<Suspense fallback={null}>
				<GLTFModel
					url="/models/component.gltf"
					position={[8, 4, -12]}
					scale={0.8}
				...
				/>
				<GLTFModel
					url="/models/generated-model.gltf"
					position={[-10, -3, -15]}
					scale={0.5}
					emissiveColor="#FF3C93"
					floatIntensity={2}
				/>
				<GLTFModel
					url="/models/material-asset-1765214009138.glb"
					position={[0, -5, -18]}
					scale={1.2}
